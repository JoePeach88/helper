...
def windows_installer(structure: list, requirements: list):
    def append_item(item: Path, requirements: bool = False):
        ...
        if item.is_dir():
            ...
            if f"\t[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String(\"{b64}\")) | Out-File -FilePath $location/{item.as_posix()} 2>> install.log\n\n" not in commands:
    ...
    installer_path = Path('./setup (Windows).ps1')
    ...
    def append_item(item: Path, requirements: bool = False):
        ...
        if item.is_dir():
            ...
            if requirements:
                ...
                with open(item, "rb") as f:
                    b64 = base64.b64encode(f.read()).decode("utf-8")
            if f"""\tbase64 --decode << 'EOF' > $location/{item.as_posix()}
{b64}
EOF
2>> install.log
""" not in commands:
...
""")
    ...
    for system in systems.keys():
        ...
        installer_path = Path(f'./setup ({system}).sh')
        ...
        commands.extend([
            ...
            f"\t\techo $PATH | grep -q \"$install_location\" && echo \"export PATH=\\\"\$PATH:$install_location\\\"\" >> ~/.{systems[system]} && source ~/.{systems[system]}\n",
            "\t\tchmod +x $install_location/helper\n",
            "\t\techo \"Usage:\n",
            "\thelper version - to view information about helper cli version and it installed modules.\n",
            "\thelper debug - to view cli debug information.\n",
            "\thelper modules ls - to view all available modules.\n",
